...
check('name').trim().not().isEmpty().withMessage('Name is missing !!'),
 ...
 .withMessage('Password must be 8 character to 20 character long.')
 ];

 exports.validatePassword = [check('newPassword').trim().not().isEmpty().withMessage('Password is missing !!')
  .isLength({min: 8, max: 20})
  .withMessage('Password must be 8 character to 20 character long.')];
...
.custom(({ url, public_id }) => {
  try {
    ...
    if (!result.protocol.includes("http"))
      throw Error("Trailer url does not include http!");
    ...
    throw Error("Trailer url is invalid from catch block!");
  }
});

exports.validateRatings = check("rating" , "Rating must be a number between 0 to 10").isFloat({min: 0, max: 10});
